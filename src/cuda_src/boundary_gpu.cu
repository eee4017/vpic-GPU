    ...
    LIST_FOR_EACH( sp, sp_list ) {
      ...
      for( ; nm; pm--, nm-- ) {
        ...
        if( nn==absorb_particles ) {
          ...
          //*** accumulate_rhob( f, p+(nm-1), g, sp_q );
          //--- goto backfill;
          //*** goto next;
        }
...
#         ifdef V4_ACCELERATION
          ...
          //*** copy_4x1( &pi->dx,    &p[nm-1].dx  );
          //*** copy_4x1( &pi->ux,    &p[nm-1].ux  );
          //*** copy_4x1( &pi->dispx, &pm->dispx );
#         else
          ...
          //*** pi->dx=p[nm-1].dx; pi->dy=p[nm-1].dy; pi->dz=p[nm-1].dz;
          //*** pi->ux=p[nm-1].ux; pi->uy=p[nm-1].uy; pi->uz=p[nm-1].uz; pi->w=p[nm-1].w;
          //*** pi->dispx = pm->dispx; pi->dispy = pm->dispy; pi->dispz = pm->dispz;
#         endif
        ...
        if( (nn>=0) & (nn<nb) ) {
          ...
          //---                           ci+n_ci, 1, face );
          //*** n_ci += pbc_interact[nn]( pbc_params[nn], sp, p+(nm-1), pm,
          //***                           ci+n_ci, 1, face );
          //--- goto backfill;
          //*** goto next;
        }
...
#     endif
    ...
    //*** local temp storage of received pi
    //*** particle_t * RESTRICT ALIGNED(32) temp_p[num_species( sp_list )];
    //*** allocate memory here

    //*** local pi count
    //*** int local_pi_cnt[num_species( sp_list )];
...
#       ifdef V4_ACCELERATION
        ...
        //*** copy_4x1(  &p[local_pi_cnt[id]].dx,    &pi->dx    );
        //*** copy_4x1(  &p[local_pi_cnt[id]].ux,    &pi->ux    );
#       else
        //--- p[np].dx=pi->dx; p[np].dy=pi->dy; p[np].dz=pi->dz; p[np].i=pi->i;
        //--- p[np].ux=pi->ux; p[np].uy=pi->uy; p[np].uz=pi->uz; p[np].w=pi->w;
        //*** p[local_pi_cnt[id]].dx=pi->dx; p[local_pi_cnt[id]].dy=pi->dy; p[local_pi_cnt[id]].dz=pi->dz; p[local_pi_cnt[id]].i=pi->i;
        //*** p[local_pi_cnt[id]].ux=pi->ux; p[local_pi_cnt[id]].uy=pi->uy; p[local_pi_cnt[id]].uz=pi->uz; p[local_pi_cnt[id]].w=pi->w;
#       endif
        //--- sp_np[id] = np+1;
        //*** sp_np[id] = sp_np[id] + 1;
